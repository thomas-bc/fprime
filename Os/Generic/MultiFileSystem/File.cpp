...
MultiFile::MultiFile(const MultiFile& other) : m_handle(other.m_handle) {}
...
MultiFile::Status MultiFile::open(const char* path, Mode mode, OverwriteType overwrite) {
    ...
    Status status = this->m_handle.m_file_sub_delegate->open(path_after_prefix, mode, overwrite);
    if (status != Status::OP_OK) {
        this->m_handle.m_file_sub_delegate->close();
        this->m_handle.m_file_sub_delegate->~FileInterface();
        this->m_handle.m_file_sub_delegate = nullptr;
    }
    return status;
}

void MultiFile::close() {
    if (this->m_handle.m_file_sub_delegate != nullptr) {
        this->m_handle.m_file_sub_delegate->close();
        this->m_handle.m_file_sub_delegate->~FileInterface();
        this->m_handle.m_file_sub_delegate = nullptr;
    }
...
MultiFile::Status MultiFile::size(FwSizeType& size_result) {
    ...
    return this->m_handle.m_file_sub_delegate->size(size_result);
}
...
MultiFile::Status MultiFile::position(FwSizeType& position_result) {
    ...
    return this->m_handle.m_file_sub_delegate->position(position_result);
}
...
MultiFile::Status MultiFile::preallocate(FwSizeType offset, FwSizeType length) {
    ...
    return this->m_handle.m_file_sub_delegate->preallocate(offset, length);
}
...
MultiFile::Status MultiFile::seek(FwSignedSizeType offset, SeekType seekType) {
    ...
    return this->m_handle.m_file_sub_delegate->seek(offset, seekType);
}
...
MultiFile::Status MultiFile::flush() {
    ...
    return this->m_handle.m_file_sub_delegate->flush();
}
...
MultiFile::Status MultiFile::read(U8* buffer, FwSizeType& size, WaitType wait) {
    ...
    return this->m_handle.m_file_sub_delegate->read(buffer, size, wait);
}
...
MultiFile::Status MultiFile::write(const U8* buffer, FwSizeType& size, WaitType wait) {
    ...
    return this->m_handle.m_file_sub_delegate->write(buffer, size, wait);
}
